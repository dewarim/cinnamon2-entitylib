...
//		getSession().flush();
    ...
    public List<Group> list() {
        Query q = getSession().createQuery("select g from Group g");
    ...
    public Group findByUser(User user) {
        Query q = getSession().createQuery("select g from Group g, GroupUser gu where g.is_user=true and gu.user.id=:id and g.id=gu.group.id");
    ...
    public List<Group> findAllByParent(Group group) {
        Query q = getSession().createQuery("select g from Group g where g.parent=:parent");
    ...
    public List<User> findAllUsers(Group group) {
        Query q = getSession().createQuery("select gu.user from GroupUser gu where gu.group=:group");
    ...
    public Group findByName(String name) {
        Query q = getSession().createQuery("select g from Group g where name=:name");
