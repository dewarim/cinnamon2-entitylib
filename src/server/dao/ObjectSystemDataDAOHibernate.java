...
public class ObjectSystemDataDAOHibernate extends
    ...
    public void delete(ObjectSystemData osd, Boolean killDescendants, Boolean removeLeftRelations){
        ...
        Query linkQuery = em.createQuery("select l from Link l where l.osd=:osd");
        linkQuery.setParameter("osd",osd);
        List<Link> links = linkQuery.getResultList();
        for(Link link : links){
            em.remove(link);
        }
    	...
    	if(predecessor != null){
            ...
            // find latestHead
            Query q = getSession().createQuery("select o from ObjectSystemData o where o.root=:root and o.latestHead=true");
            q.setParameter("root", predecessor.getRoot());
            List<ObjectSystemData> latestHead = q.getResultList();
            log.debug("size of latestHeadList:"+latestHead.size());
    		if(! predecessor.getVersion().contains(".") && latestHead.isEmpty()){
