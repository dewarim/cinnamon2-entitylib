...
@NamedQueries(
		{
			...
			@NamedQuery(
					...
					query= "SELECT gu FROM GroupUser gu WHERE user=:user AND group=:group"
			),
			...
			@NamedQuery(
					...
					query = "select o from ObjectSystemData o where o.predecessor=:predecessor order by o.id desc"
			),
			@NamedQuery(
			...
			@NamedQuery(
					...
					query = "select r from Relation r where r.type=:type"
			),
			@NamedQuery(
			...
			@NamedQuery(
					...
					query = "select r from Relation r where r.leftOSD=:left and r.type=:type"
			),
			@NamedQuery(
					name = "findRelationsByRightAndType",
					query = "select r from Relation r where r.rightOSD=:right and r.type=:type"
			),			
			@NamedQuery(
